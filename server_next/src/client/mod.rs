...
impl ClientStates {
    ...
    pub fn refresh(&mut self) {
        self.busy = self.registry.clone();
        for id in &self.ignore {
            self.busy.remove(id);
        }
    }

    pub fn register(&mut self, uuid: String) {
        self.registry.insert(uuid);
    }

    pub fn delist(&mut self, uuid: String) {
        self.registry.remove(&uuid);
        self.busy.remove(&uuid);
        self.ignore.remove(&uuid);
    }

    pub fn update(&mut self, uuid: String) {
        self.busy.remove(&uuid);
    }

    pub fn ignore(&mut self, uuid: String) {
        self.busy.remove(&uuid);
        self.ignore.insert(uuid);
    }

    pub fn unignore(&mut self, uuid: String) {
        self.ignore.remove(&uuid);
    }
